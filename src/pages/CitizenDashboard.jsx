...
import { getUserGrievances } from "@/services/grievanceService";
...
const CitizenDashboard = () => {
  ...
  useEffect(() => {
    const fetchGrievances = async () => {
      if (!user) {
        console.log('[CitizenDashboard] No user found.');
        return;
      }
      console.log('[CitizenDashboard] Fetching grievances for userId:', user.id);
      try {
        setLoading(true);
        const userGrievances = await getUserGrievances(user.id);
        setGrievances(userGrievances);
        console.log('[CitizenDashboard] Set grievances:', userGrievances);
      } catch (error) {
  ...
  const stats = [
    ...
    { title: "In Progress", value: grievances.filter(g => g.status === 'in-progress').length.toString(), icon: AlertCircle, color: "bg-orange-600" },
  ];
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <div className="mb-6 lg:mb-8">
        ...
        <p className="mt-2 text-gray-600 text-sm lg:text-base">Welcome back! Here's an overview of your grievances.</p>
      </div>
      ...
      <div className="mb-6 lg:mb-8">
        <Link
          to="/submit"
          className="inline-flex items-center px-4 lg:px-6 py-2 lg:py-3 border border-transparent text-sm lg:text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-colors w-full sm:w-auto justify-center"
        >
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="px-4 lg:px-6 py-4 border-b border-gray-200">
          <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <h3 className="text-lg font-medium text-gray-900">Your Grievances</h3>
              ...
              <div key={grievance.id} className="p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{grievance.id}</p>
                  </div>
                ...
                <p className="text-xs text-gray-500">{formatDate(grievance.createdAt)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {filteredGrievances.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              {grievances.length === 0 
                ? "No grievances submitted yet. Submit your first grievance!" 
